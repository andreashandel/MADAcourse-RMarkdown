...
In preparation for next week, make a copy of the `tidytuesday_exercise.Rmd` file, name it `tidytuesday_exercise2.Rmd`. Open it, delete everything apart from the YAML header (the stuff between the `---`). Add a sentence that says "To be filled" or something like that. You wll fill it next week.
...
And if you've really gotten into making websites with R Markdown, you can always move to [blogdown](https://bookdown.org/yihui/blogdown/). This is a much more flexible and powerful way of creating websites, but it's also more complex. Maybe the currently best starting point for blogdown is [this online post/tutorial](https://www.apreshill.com/blog/2020-12-new-year-new-blogdown/).
