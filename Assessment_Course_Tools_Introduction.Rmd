...
If you haven't done so, please first read through the [**Assessments**](./Course_Assessments.html) page before continuing here.
...
Note that the quiz and grading system will only become available once student passwords have been set.
...
It is possible to change the looks of your website. Full styling will require knowledge of CSS and HTML. We won't get into this. But we'll explore a quick way to change the look. Open the `_quarto.yml` file. This file defines the menu structure, and some styling. To change the look, can replace `theme` with a different one. [Many themes are available](https://quarto.org/docs/output-formats/html-themes.html). Try some, see how things change. 
...
You can do a lot more styling by for instance editing the `styles.css` file. This is not a web design class `r emoji::emoji('grin')`, we therefore won't go into styling further, but feel free to explore on your own.
