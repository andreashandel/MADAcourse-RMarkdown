...
We also want to code the 3 ordinal/multi-level factors as ordered, so make sure they are coded as ordered factors. The order should of course be None/Mild/Moderate/Severe.
...
You should end up with a data frame that has 730 observations and 26 variables. This is the dataset we'll use now for modeling.
...
* Create a recipe for the data and fitting. You won't need to do much, just make sure you code the categorical variables as dummy variables, otherwise things might not work smoothly. For that, you want to use the `step_dummy` function and pick all nominal predictor variables (which are actually all variables here, since the only continuous variable is our outcome).
...
Once you have done the tuning, you can take a look at some diagnostics by sending your object returned from the `tune_grid()` function to `autoplot()`. For instance if you tuned the tree and saved the result uas  `tree_tune_res`, you can run `tree_tune_res %>% autoplot()`. Depending on the model, the plot will be different, but in general it shows you what happened during the tuning process.
