...
# Overview
This very short unit re-visits all of the data analysis concepts and components we discussed so far, and provides summary and big-picture discussion, meant to wrap up things.
...
# Introduction
Earlier in the course, we covered the whole data analysis workflow and all its components in a big-picture, forward-looking manner. Back then, the topics were abstract. At this stage, we have covered each topic and worked on specific examples. Now is a good time to [re-read the earlier unit.](./Data_Analysis_Overview.html) 

Hopefully, upon re-reading, some of the topics that were theoretical and abstract back when you first read it are no clearer and more concrete.
...
While I hope you appreciate by now that every data analysis is different, and you need to make lots of decisions along the way for which there is often more than one possible reasonable choice, there are some components that I think are needed for any high quality project. Those can be seen as the basic ingredients of a good data analysis, with lots of freedom within each category. 

:::note
I mentioned this point previously, probably more than once `r emoji::emoji('smile')`: This need to make choices and decisions on how exactly to perform various steps of an analysis is one reason why p-values are generally meaningless, or at least do not mean what people think/claim they mean. For a nice discussion on this topic, see [this unpublished paper by Gelman](http://www.stat.columbia.edu/~gelman/research/unpublished/p_hacking.pdf), which nicely, and in an easy to understand manner, describes the problem in detail. I mentioned this before as [**Researcher's degress of freedom**](https://en.wikipedia.org/wiki/Researcher_degrees_of_freedom), which is also related to [Data Dredging](https://en.wikipedia.org/wiki/Data_dredging) and [_p-hacking_](https://doi.org/10.1371/journal.pbio.1002106).
:::
...
It might be worth revisit some of the older materials. For instance, you might want to give [_The Art of Data Science (ADS)_](https://leanpub.com/artofdatascience) another go-through. It addresses these various topics from a fairly big-picture level and will give you some additiona useful pointers. 
