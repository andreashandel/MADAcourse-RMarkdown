...
* In the future, if you want to edit your introduction site/project, click or double-click on the `.Rproj` file. You can try it. Close RStudio, click the `.Rproj` file and the same setup as before should show up. Creating new projects as specific R projects have several advantages. One is that it makes how you write file paths easier, so while having an `.Rproj` file is not strictly required I recommend you do it that way.
...
* Finally, include a link to a picture/video/website/etc. The linked material should be somehow related to data analysis and that you find really cool/interesting/weird/etc. Provide a few explanatory sentences.
...
Save the file you just created as **index.Rmd** in the repository folder. You need to give it this name; otherwise, the Github website display won't work. Then knit the file, you should see an index.html file show up.
...
* If you like how fast you were able to create your own small website and want to learn more, two good resources are [the blogdown book](https://bookdown.org/yihui/blogdown/) and [the Rmarkdown website chapter](https://bookdown.org/yihui/rmarkdown/rmarkdown-site.html) in the Rmarkdown book. For my group website, I use blogdown/hugo and a theme called Academic. You can look at all the files [here](https://github.com/ahgroup/ahgroupwebsite). The course website is more straightforward and uses the RStudio/Rmarkdown website setup. You can look at all these files [here](https://github.com/andreashandel/MADAcourse).
