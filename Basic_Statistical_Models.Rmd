...
* Know about several types of basic models.
...
In further modules, we look at several more complex models. Most of the time, you do not understand every detail of a model. You need to understand enough about how the model works, its strengths and limitations, to determine if a certain model is useful for a given situation or not. 
...
Another good overview and introduction, at a slightly more abstract level, is given in the [linear regression chapter in ISLR](https://statlearning.com/). You should read through this chapter in at least enough detail to get the big picture ideas. Depending on your interest/willingness to look at equations, try to follow along with the math as well. But the important part is to understand the overall ideas.
...
As you can tell, this is such a fundamental type of model it is covered in a lot of resources. Definitely check the R4DS source, skim through ISLR, then take a look at the other ones I list here or any other source you find and like (let me know if you found a nice free one so I can add it to the resources list. Make sure you read and learn enough about these models that you feel comfortable understanding conceptually what they are used for and how they work.


# Simple models for categorical outcomes (classification)

For categorical outcomes, at least those that come in 2 categories, logistic regression is the most common approach. Logistic regression has a bit of an unfortunate name since it is used for **classification** even though we call it a **regression** model. In some sense it is actually a regression model, since logistic regression predicts a numeric value between 0 and 1, which is the probability of having the outcome or not. We then usually (but not always) decide that 0.5 is the cut-off, and call any probability below as 0/not present/no and above that as 1/present/yes. Just keep in mind that logistic regression is used for classification.
...
GLM all have a similar structure, namely there is a linear combination of predictor variables (e.g., $b_1*age + b_2*height$) and those are connected to the outcome through what is called a **link function**. GLM are a very useful class of models, of course the most used ones are the two we just discussed. At the end of the [classification chapter in ISLR](https://statlearning.com/) is a nice brief discussion of GLM, I suggest you read through that in enough depth to get the overall ideas, no need to go through all the math.
...
There isn't really one single, standard go-to method for unsupervised learning - in contrast to the very common linear and logistic models for supervised learning. There are often used methods, such as **Principal Component Analysis (PCA)** and a group of methods called **Clustering methods**. Since most of you will work on labeled data using supervised methods most of the time, we won't go much into unsupervised methods. If you want to learn a bit more, check out for instance the
