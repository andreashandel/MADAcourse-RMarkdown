...
[Generalized additive models (GAMs)](https://en.wikipedia.org/wiki/Generalized_additive_model) are in some sense (though not completely, see below) a general form of the different models just discussed. The preserve the feature that each predictor enters the model additively. But now, each predictor can be related to the outcome with a -- potentially complicated, potentially unknown -- function. In equation form, a GAM can be written as:

$$g(Y) = x_0 + f_1(x1) + f_2(x2) + \ldots + f_n(x_n).$$
Here, the $f_n$ are functions of the precictor $x_n$, and $g()$ can again be some overall link function.
