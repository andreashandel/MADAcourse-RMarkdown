...
d <- d %>% mutate(gg2c4 = fct_recode(gg2c4, "No" = ""  ))
str(d$gg2c4)
```
...
#convert any other variables as needed
...
Next, we remove the `Unspecified` entry in `Hemisphere` and recode `Action1` and `Path1` as described in the Data Analysis script. We also, do the same grouping Restaurant/Other with the `Setting_1` variable. Again, remember that there are `restaurant` and `Restaurant` values, so you need to fix that too.
...
p1 <- d %>% select_if(is.numeric) %>% mutate(G24 = d$gg2c4) %>%
            gather(-G24, key = "var", value = "value") %>% 
            ggplot(aes(x = G24, y=value)) + geom_point()  + geom_violin() +
...
p1 <- d %>% select_if(is.factor) %>%
            gather(-gg2c4, key = "var", value = "value") %>% 
            ggplot(aes(x = value, y=gg2c4)) + geom_count()  +
            facet_wrap(~ var, scales = "free") + theme_bw()
print(p1)
```
...
d %>% select_if(is.factor) %>% summary()
```

You should see that there is only a single entry for _No_ in `Action1`, and small entries for _Multiple_ and _Unspecified_ in `Country` and _Environmental_ in `Trans1`. We'll address that as follows: Remove the observation for which action is _No_, combine Countries into 3 groups Japan/USA/Other, and since I don't even know what biologically the difference is between _Environmental_ and _Waterborne_ transmission (seems the same route to me based on my norovirus knowledge), we'll move the Waterborne into the environmental. Finally, I noticed there is a blank category for season. That likely means it wasn't stated in the paper. Let's recode that as _Unknown_. Finally, re-order data such that the outcome is the first column. 


```{r, more-factor-cleaning}
#write code as described above
d <- d %>% dplyr::filter(Action1 != "No") %>% 
            dplyr::mutate(Country = fct_recode(Country,
                                     "Other" = "Multiple",
                                     "Other" = "Unspecified")) %>%
            dplyr::mutate(Trans1 = fct_recode(Trans1,
                                     "Environmental" = "Waterborne")) %>% 
            dplyr::mutate(season = fct_recode(season,
                                     "Unknown" = "")) %>% 
            dplyr::select(gg2c4, everything() ) %>%
            droplevels() 
summary(d)
dim(d)
```
...
We could do data splitting again as we did in the previous exercise, to have a final test set. But since you saw how it works in the last exercise, we skip it here. We use the full data to fit to our models. We'll still use cross-validation to get a more honest estimate of model performance. For a real data analysis, the choice to keep some for a final test or not is based on your goals. If your focus is predictive performance, you should consider this split. If your focus is inference or exploratory analysis, you might want to skip this.
...
Since doing the subset selection below starts getting slow, and because it's a good topic to know about, we are going to use  parallelization here. `mlr` uses the package `parallelMap` for this.
...
Some setup settings used in various bits of code below.

Some setup settings that are used in various code chunks below. 

```{r mlr-setup}
#some setup to specify the problem and approach for mlr
outcome <- d$gg2c4 
outcomename = "gg2c4"
predictors <- d[,-1]
...
null_pred <- factor(rep("No",length(outcome)), levels = c("No","Yes")) 
nulltable = table(outcome,null_pred) #compute 2x2 table/confusionmatrix for null model 
null_acc = (nulltable[1,1]+nulltable[2,2])/sum(nulltable)
#an alternative is null_acc =  sum(null_pred == outcome)/length(null_pred)
print(null_acc)
```
...
null_auc = pROC::auc(as.numeric(outcome)-1,as.numeric(null_pred)-1)
print(null_auc)
```
...
for (nn in 1:npred)
{
    ...
    ## Generate the task, i.e. define outcome and predictors to be fit
    mytask = makeClassifTask(id='unianalysis', data = unidata, target = outcomename, positive = "Yes")
    model = resample(mylearner, task = mytask, resampling = sampling_choice, show.info = FALSE, measures = auc )
    unifmat[nn,1] = names(predictors)[nn] 
    unifmat[nn,2] = model$aggr
}
kable(unifmat)
