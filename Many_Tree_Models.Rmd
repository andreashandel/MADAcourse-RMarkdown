...
Many of these methods and packages are similar and differ in the details of the model implementation, the speed at which they run, if one can parallelize them, etc. Some common ones that people use are `rpart` for individual trees, `ranger` for random forests, `xgboost` for boosting. Many others are available too. Often it doesn't matter much which package you use. Sometimes you might need specific features that only a certain package can give you, then you need to go with that one. 
