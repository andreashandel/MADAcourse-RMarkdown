...
Here are some of the most prominent ones:
  ...
  * The [`readr` package](https://readr.tidyverse.org/index.html) is great first choice for importing CSV and similar spreadsheet type data. For some more introductory material on the `readr` package, see [chapter 11 of R4DS](https://r4ds.had.co.nz/data-import.html). It can often guess the right way to import data, even if it is slightly messy. But always check! It's a good first choice for CSV/TSV/text style data.
...
* The folks from Posit/RStudio but together a useful [data import cheat sheet](https://github.com/rstudio/cheatsheets/blob/master/data-import.pdf), which is worth checking out.
