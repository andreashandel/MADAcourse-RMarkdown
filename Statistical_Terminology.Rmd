...
For additional attempts at defining terms related to data analysis, see the [Glossary page](./Course_Glossary.html) - and certainly feel free to contribute!
...
In __Machine Learning (ML)__, the models tend to be more complex, and the goal is mainly on getting a "powerful" model, i.e., a model that is good at prediction. Understanding how different inputs lead to different outcomes is of secondary importance. Data is often abundant, so more complex models can be used. 

__Artificial intelligence (AI)__ can be considered as a type of machine learning. The types of complex models that dominate AI these days are generally based on neural nets. Neural nets sound fancy, and they are certainly quite complex, but technically speaking, you can actually think of them as just a large collection of logistic regression type models combined together.


As mentioned before, terminology is fuzzy. Thus, a fairly simple model like a linear or logistic regression model could be considered a "classical" statistical model, while a more complex support vector machine or random forest (we'll visit those later) are generally considered machine learning models. However, the terminology is poorly-defined, and you will see pretty much any approach can be given any label. Often it makes sense to think of the two terms as describing more or less the same thing, and that is applying some kind of mathematical or computational model to gain insight from the data. For some similar ideas, and a bit more details, [read this blog post](https://www.sharpsightlabs.com/blog/difference-machine-learning-statistics-data-mining/).
