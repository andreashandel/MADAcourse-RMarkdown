...
* Remove all variables that have _Score_ or _Total_ or _FluA_ or _FluB_ or _Dxname_ or _Activity_ in their name. Don't do this manually one by one, figure out how to use R commands that let you remove things in an efficient manner, using e.g., `contains()` or `starts_with()` from `dplyr/tidyselect`.
...
At the end of your (fairly short) cleaning process, you should end up with 730 observations and 32 variables. Save the new data set as `RDS` file into the appropriate folder (either `data` or if you made it, your clean data folder). (For single objects [`RDS` is better than `RData/RDA`.](https://rstudio-education.github.io/hopr/dataio.html#r-files) )   

For our further analysis, we decide (well, I'm deciding that for you `r emoji::emoji('grin')`) that our main continuous outcome of interest is **Body temperature**, and our main categorical outcome is **Nausea**. We want to see if the other symptoms are correlated with (predict) those outcomes.
...
**Remember that YAML is very picky about indentation, so if you get strange error messages, it is often likely that your entries are not aligned/indented exactly right. 
If you suspect that you have this problem, you can show whitespace characters in RStudio by going to `Tools -> Global Options -> Code -> Display" and check _Show Whitespace Characters_.
**
