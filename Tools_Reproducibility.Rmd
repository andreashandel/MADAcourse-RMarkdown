...
Note that if you want to produce pdf output, you need to have a (La)TeX system installed. It's a free typesetting system that is a bit similar to Markdown, but more complicated and more powerful. The [currently recommended LaTeX system is TinyTeX](https://bookdown.org/yihui/rmarkdown-cookbook/install-latex.html). Others are [MiKTeX](https://miktex.org/) for Windows (which I generally use) and [MacTeX ](http://www.tug.org/mactex/) for Mac. We don't absolutely need LaTeX it for the class, but it's good to have if you ever want to create pdf output, and it is and free, so I suggest you go ahead and install it.
