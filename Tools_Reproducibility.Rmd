...
Creating a reproducible and automated analysis used to be a good bit of extra work, but not anymore. Quarto, GitHub, and related tools have made it fairly easy to set up a reproducible workflow. We discuss GitHub separately, see that document. Since it controls and tracks any changes you make, and works nicely with collaborators, it is an excellent tool for reproducible work. 

While there are different tools and programming languages that allow reproducible research (e.g. Jupyter notebooks in Python, Mathematica notebooks, Sweave, Latex), we will focus on one tool, namely the [Quarto system](https://quarto.org/). 

Quarto was created by the folks at Posit (formerly RStudio), and is a newer and improved iteration of R Markdown.

The idea is that you can easily combine code with plain text documents with simple formatting (called Markdown format), and then turn them into a lot of different output formats, e.g., HTML, PDF, Word, or slides. You can apply layout and styling to those documents, which is done separately from the content. This means you can quickly switch between outputs. In our flow, the software in the background that turns our text documents into different formats is QUarto, which in turn [calls various other pieces of software](https://quarto.org/docs/get-started/hello/rstudio.html#how-it-works).

The good thing is, you don't need to care, it all happens (almost always) smoothly behind the scenes. Quarto and markdown are rather easy to learn. If you have no experience with Markdown, I suggest you go through [this nice, short interactive tutorial.](https://www.markdowntutorial.com/). A good reference to look up formatting for Markdown until you have it memorized is this [online cheat sheet](https://github.com/adam-p/markdown-here/wiki/Markdown-Cheatsheet). 

The folks from Rstudio developed [R Markdown](http://rmarkdown.rstudio.com/). This allows you to combine R code with Markdown text. More recently, they developed [Quarto](https://quarto.org/), which is an improved version of the whole system that allows using different programming languages and creating many different types of outputs (papers, presentations, websites, etc.). In this course, we focus on using Quarto. But you'll still see people use R Markdown. As an example, this whole course is written in R Markdown and [lives on GitHub](https://github.com/andreashandel/MADAcourse). Eventually I'll switch it to Quarto. The good thing is that the transition is easy, it's almost the same. Quarto is the future, so it's a good idea to start using it.

The whole Quarto system has become incredibly flexible and powerful, and we'll use it in this class. Because it is very feature rich and you can do a ton, trying to read all about it does not make much sense. The best is a _learn it as you need it_ approach. The [Quarto website](https://quarto.org/) has great documentation, you'll likely be going there often.
